...
export default class OverPassCache {
    ...
    process(theme, layer, next, retry, setSuccess, setError, setDeletedFeatures) {
        ...
        .catch((status, responseText) => {
            if (status === 429) {
                ...
                logger.error(`OverPass said « Too many requests » ${this._tooManyRequestsTriesInARow} times`);
            ...
            let error;
            const overPassJson = JSON.parse(responseText);
    ...
    static _retrieveData(uri) {
        ...
        const timeout = parseInt(
            config.get('client.overPassTimeout'),
            10
        );
    ...
    static _findDeletedFeatures(themeFragment, layerUuid, overPassResult) {
        ...
        return OverPassCache._extractDeletedFeatures(
            oldGeoJson,
            overPassGeoJson
        );
    }
