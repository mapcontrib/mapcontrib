...
export default class OverPassCache {
    ...
    static process(theme, layer, nextCallback, retryCallback, setSuccess, setError) {
        ...
        .then((data) => {
            if ( config.get('client.overPassCacheDeleteArchiveEnabled') ) {
                OverPassCache._saveDeletedPoiInDatabase(
                    theme.fragment,
                    layer.uuid,
                    data
                );
    ...
    static _saveDeletedPoiInDatabase(themeFragment, layerUuid, overPassResult) {
        logger.debug('_saveDeletedPoiInDatabase');
        ...
        return new Promise((resolve) => {
            ...
            fs.readFile(
                ...
                (data) => {
                    ...
                    if ( !_.isEqual(oldGeoJson, overPassGeoJson) ) {
                        console.log('memes resultats');
                    }
                    else {
                        console.log('pas les memes resultats');
                    }
