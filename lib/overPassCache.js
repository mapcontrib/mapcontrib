    ...
    OverPassCache._retrieveData(url)
      .then(data => {
        ...
        if (OverPassCache.isCacheArchiveEnabled(layerModel)) {
          ...
          const modifiedFeatures = OverPassCache._readStateCacheFile(
            fragment,
            layerUuid,
            'modified'
          );
          const deletedFeatures = OverPassCache._readStateCacheFile(
            fragment,
            layerUuid,
            'deleted'
          );
          const archivedFeatures = OverPassCache._readStateCacheFile(
            fragment,
            layerUuid,
            'archived'
          );
          ...
          overPassGeoJson.features = overPassGeoJson.features.map(feature => {
            ...
            if (modifiedFeaturesIds.indexOf(feature.id) > -1) {
