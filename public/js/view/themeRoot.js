...
export default Marionette.LayoutView.extend({
  ...
  onRender() {
    ...
    const fullscreenSupport = FullscreenHelper.isFullscreenAPISupported(
      this._document
    );
    ...
    FullscreenHelper.onFullscreenChange(window, () => {
      const fullscreenElement = FullscreenHelper.getFullscreenElement(
        this._document
      );
  ...
  onShow() {
    ...
    _.each(
      this._layerCollection.getVisibleLayers(),
      layerModel => {
  ...
  setTileLayer(id) {
    ...
    this._currentTileMinZoom = tile.minZoom;
    this._currentTileMaxZoom = tile.maxZoom;

    if (currentZoom > tile.maxZoom) {
      this._map.setZoom(tile.maxZoom);
    }
  ...
  addTempGpxLayer(layerModel, fileContent) {
    ...
    const layer = Omnivore.gpx.parse(fileContent);
  ...
  addTempCsvLayer(layerModel, fileContent) {
    const rootLayer = this._buildRootLayer(layerModel);
    const layer = Omnivore.csv.parse(fileContent);
  ...
  addTempGeoJsonLayer(layerModel, fileContent) {
    const rootLayer = this._buildRootLayer(layerModel);
    const layer = L.geoJson(JSON.parse(fileContent));
  ...
  addGpxLayer(layerModel, hiddenLayer) {
    Omnivore.gpx(layerModel.get('fileUri'))
      .on('error', xhr => {
  ...
  addCsvLayer(layerModel, hiddenLayer) {
    Omnivore.csv(layerModel.get('fileUri'))
      .on('error', xhr => {
        if (xhr.error.status === 404) {
          new CsvErrorNotificationView({
            model: layerModel,
  ...
  addGeoJsonLayer(layerModel, hiddenLayer) {
    Omnivore.geojson(layerModel.get('fileUri'))
      .on('error', xhr => {
        if (xhr.error.status === 404) {
          new GeoJsonErrorNotificationView({
            model: layerModel,
  ...
  updateMarkerStyle(layerModel) {
    ...
    for (const layer of layers) {
      switch (layer.toGeoJSON().geometry.type) {
        ...
        case 'LineString':
        case 'MultiLineString':
          layer.setStyle(MapUi.buildLayerPolylineStyle(layerModel));
          break;
        case 'Polygon':
        case 'MultiPolygon':
          layer.setStyle(MapUi.buildLayerPolygonStyle(layerModel));
          break;
        default:
          layer.refreshIconOptions(
            MapUi.buildMarkerLayerIconOptions(layerModel)
  ...
  updateHeatStyle(layerModel) {
    ...
    rootLayer.setOptions(MapUi.buildHeatLayerOptions(layerModel));
      ...
      if (layer.feature.id === osmId) {
        ...
        if (layer._popup) {
          layer._popup.setContent(
            this._buildLayerPopupContent(layer, layerModel, {
              properties: {
  ...
  renderUserButton() {
    if (!this._app.isLogged()) {
      ...
      if (avatar) {
        ...
        this.ui.userButton.removeClass('avatar').html(letters);
      }
    }
  },
  ...
  onClickZoomIn() {
    ...
    if (currentZoom === this._currentTileMaxZoom) {
      return;
    }
  ...
  onClickZoomOut() {
    ...
    if (currentZoom === this._currentTileMinZoom) {
      return;
    }
  ...
  onZoomEnd() {
    this.ui.toolbarZoomLevel.text(this._map.getZoom());
  ...
  updateMinDataZoom() {
    if (this._layerCollection.models.length === 0) {
      ...
      for (const layerModel of this._layerCollection.models) {
        ...
        if (
          layerModel.get('cache') === true &&
          layerModel.get('cacheUpdateSuccess')
        ) {
          continue;
        }
  ...
  onClickExpandScreen() {
    FullscreenHelper.requestFullscreen(this._document.documentElement);
  },
