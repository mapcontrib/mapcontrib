...
export default Marionette.ItemView.extend({
  ...
  ui: {
    ...
    layerCacheArchive: '#layer_cache_archive',
    ...
    cacheArchiveInfo: '.info_cache_archive_btn',
  ...
  events: {
    'change @ui.layerCache': 'onChangeLayerCache',
  ...
  onRender() {
    ...
    this.ui.layerCacheArchive.prop('checked', this.model.get('cacheArchive'));

    if (!this.model.get('cache')) {
      this.ui.layerCacheArchive.prop('disabled', true);
    }
    ...
    if (this.model.get('cacheUpdateDate')) {
      ...
      const date = moment
        .utc(this.model.get('cacheUpdateDate'))
        .tz(timezone)
        .fromNow();

      this.ui.cacheDate
        .html(document.l10n.getSync('editLayerFormColumn_cacheDate', { date }))
        .removeClass('hide');
    }
    ...
    this.ui.cacheArchiveInfo.popover({
      ...infoOptions,
      title: document.l10n.getSync(
        'editLayerFormColumn_cacheArchivePopoverTitle'
      ),
      content: MarkedHelper.render(
        document.l10n.getSync('editLayerFormColumn_cacheArchivePopoverContent')
      )
    });
  },
  ...
  onChangedMapZoom() {
    ...
    this.ui.currentMapZoom.html(
      document.l10n.getSync('editLayerFormColumn_currentMapZoom', {
        currentMapZoom
      })
    );
  },
  ...
  onChangeLayerCache() {
    if (this.ui.layerCache.prop('checked') === true) {
      this.ui.layerCacheArchive.prop('disabled', false);
    } else {
      this.ui.layerCacheArchive.prop('checked', false).prop('disabled', true);
    }
  },
  ...
  onSubmit(e) {
    ...
    const layerCache = this.ui.layerCache.prop('checked');
    let layerCacheArchive = this.ui.layerCacheArchive.prop('checked');

    if (!layerCache) {
      layerCacheArchive = false;
    }
    ...
    this.model.set('cache', layerCache);
    this.model.set('cacheArchive', layerCacheArchive);
    ...
    if (!this.model.get('cache')) {
      if (
        this._oldModel.get('overpassRequest') !==
        this.model.get('overpassRequest')
      ) {
        updateRequest = true;
      }
    }
      ...
      {},
      {
        success: () => {
          ...
          if (updateCache) {
            ...
            xhr.open(
              'GET',
              `${CONST.apiPath}/overPassCache/generate/${layerUuid}`,
              true
            );
            xhr.send();
          }
