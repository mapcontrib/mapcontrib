...
export default Marionette.LayoutView.extend({
  ...
  _buildNavItemsFromPresetCategoryModels(presetCategoryModels, parentModel) {
    const items = presetCategoryModels
      .filter(model => {
        const child = this._presets.findWhere({
          parentUuid: model.get('uuid')
        });
    ...
    if (parentModel) {
      backItem.callback = this._displayPresetCategoryChildren.bind(
        this,
        this._presetCategories.findWhere({
          uuid: parentModel.get('parentUuid')
        })
      );
  ...
  _filterPresets(searchString) {
    ...
    const iDPresets = this._iDPresetsHelper.buildPresetsFromSearchString(
      searchString
    );
    const presetModels = this._presets.buildPresetsFromSearchString(
      searchString
    );
